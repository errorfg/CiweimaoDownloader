...
if __name__ == "__main__":
    ...
    while True:
        ...
        # 扫描文件夹
        foundFolders = scan_folders()

        if config.setting.manualBook.enable == True:
            models.Print.info("[INFO] 手动目录模式已开启")
            queue.append("1000000")
        elif config.setting.batch.enable == True:
            if config.setting.batch.auto == False:
            ...
            models.Print.info("[INFO] 输入 'all' 转换全部，'exit' 退出程序")
            choice = models.Print.opt(f"[OPT] 请输入书籍Url/目录名：")

            if choice.lower() in ['exit', 'quit', 'q']:
                models.Print.info("[INFO] 退出程序")
                break
                ...
                if len(foundFolders) > 0:
                    ...
                    models.Print.warn("[WARN] 没有找到待转换的目录")
                    continue
            elif choice.strip() == '':
                continue
            else:
        ...
        # 处理队列
        for url in queue:
            success, skipped = convert_book(url)
            if success:
                convertedCount += 1
            if skipped:
                skippedCount += 1
        ...
        elif len(queue) == 1 and convertedCount > 0:
            models.Print.info(f"[INFO] 转换完成！")

        # 批量模式只运行一次
        if config.setting.batch.enable == True or config.setting.manualBook.enable == True:
            models.Print.opt(f"[OPT] 任意键退出程序...")
            break

        models.Print.info("")  # 空行分隔
